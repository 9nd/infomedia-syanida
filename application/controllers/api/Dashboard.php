...
class Dashboard extends CI_Controller
{
    ...
    function get_dapros_grapik()
    {
        $data['query_dapros'] = $this->trans_profiling->live_query(
            "
            SELECT count(*) as jumlah_data,sumber FROM dbprofile_validate_forcall_3p 
            WHERE (ISNULL(update_by) OR update_by = 'baru' OR update_by = 'BARU' OR update_by = '')
            AND (ISNULL(duplicate_ncli) OR duplicate_ncli = 0 OR duplicate_ncli = '') AND
         status = 0 GROUP BY sumber
            "
        );
