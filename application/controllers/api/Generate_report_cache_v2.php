...
class Generate_report_cache_v2 extends CI_Controller
{
    ...
    function update_duplicate($limit = 100, $offset = 0, $jumlah = false, $count_update = 0)
    {
        ...
        $data['link'] = base_url() . "api/Generate_report_cache_v2/update_duplicate/" . $limit . "/" . $offset . "/" . $jumlah . "/" . $count_update;
        $this->load->view('Custom_view/count_down', $data);
    }
    public function clear_cabut($limit = 100, $offset = 0, $jumlah = false, $count_update = 0)
    {
        ...
        $data['link'] = base_url() . "api/Generate_report_cache_v2/clear_cabut/" . $limit . "/" . $offset . "/" . $jumlah . "/" . $count_update;
        $this->load->view('Custom_view/count_down', $data);
    }
    public function clear_double_verified($limit = 100, $offset = 0, $jumlah = false, $count_update = 0)
    {
        ...
        $data['link'] = base_url() . "api/Generate_report_cache_v2/clear_double_verified/" . $limit . "/" . $offset . "/" . $jumlah . "/" . $count_update;
        $this->load->view('Custom_view/count_down', $data);
    }
    public function dapros_payment($limit = 100,  $jumlah = false, $count_update = 0)
    {
        
        if(!$jumlah){
            $querynum = "SELECT * FROM dapros_payment WHERE status =0 ";
            $eksekusinum= $this->db->query($querynum);
            $jumlah=$eksekusinum->num_rows();
        }
        
        $queryout = "SELECT * FROM dapros_payment WHERE status =0 ORDER BY inet DESC LIMIT $limit  ";
        $eksekusiout = $this->db->query($queryout);
        
        if ($eksekusiout->num_rows() > 0) {
            foreach ($eksekusiout->result() as $dp) {
                $status = $this->loadBillData($dp->inet);
                /**UPDATE QUERY */
                $queryupdate = "UPDATE dapros_payment SET status='".$status."' WHERE inet ='".$dp->inet."'";
                $this->db->query($queryupdate);
                $count_update++;
            }
        }
        if($jumlah <= $count_update){
            echo "Data Berhasil Diupdate :".$count_update;
        }else{
            $data['link'] = base_url() . "api/Generate_report_cache_v2/dapros_payment/" . $limit . "/" . $jumlah. "/" . $count_update;
            $this->load->view('Custom_view/count_down', $data);
        }
