...
extends CI_Controller
{
    ...
    function get_profiling_reguler()
    {
        ...
        if ($_GET['start']) {
            ...
            $query_trans_profiling = $this->trans_profiling->live_query(
                "SELECT * FROM trans_profiling  WHERE  DATE_FORMAT(lup ,'%Y-%m-%d') >= '$start' AND DATE_FORMAT(lup ,'%Y-%m-%d') <= '$end' "
            ...
            $query_trans_profiling = $this->trans_profiling_daily->live_query(
                "
                SELECT * FROM trans_profiling_daily WHERE DATE(lup) = '" . $start . "' "
        ...
        $opsi['total'] = $opsi['opsi_1'] + $opsi['opsi_2'] + $opsi['opsi_3'] + $opsi['opsi_4'];
    ...
    function get_payment()
    {
        ...
        $end = $_GET['end'];
        $start = date('Y-m-d');
        $data['query_payment'] = $this->trans_profiling->live_query(
            "SELECT
