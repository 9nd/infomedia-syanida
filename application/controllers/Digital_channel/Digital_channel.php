<?php
require APPPATH. '/controllers/Digital_channel/Digital_channel_config.php';

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Digital_channel extends CI_Controller {
   private $log_key,$log_temp,$title;
   function __construct(){
        parent::__construct();
		$this->load->model('Digital_channel/Digital_channel_model','tmodel');
		$this->log_key ='log_Digital_channel';
		$this->title = new Digital_channel_config();
   }
	...
	public function index(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_refresh_table'	=> site_url().'Digital_channel/Digital_channel/refresh_table/'.$this->_token,
			'link_create'			=> site_url().'Digital_channel/Digital_channel/create',
			'link_update'			=> site_url().'Digital_channel/Digital_channel/update',
			'link_delete'			=> site_url().'Digital_channel/Digital_channel/delete_multiple',
		);
		
		$this->template->load('Digital_channel/Digital_channel_list',$data);
	}
	...
	public function create(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url().'Digital_channel/Digital_channel/create_action',
			'link_back'				=> $this->agent->referrer(),			
		);
		
		$this->template->load('Digital_channel/Digital_channel_form',$data);
	...
	public function create_action(){
		...
		*/	
		...
		if(!$o->not_empty($val['channel'],'#channel')){
			echo $o->result();	
			return;
		}

		//mencegah data double
		$field=array('channel'=>$val['channel']);
		$exist = $this->tmodel->if_exist('',$field);
		if(!$o->not_exist($exist,'#channel')){
			echo $o->result();	
			return;
		}
		...
		//mencegah data kosong
		if(!$o->not_empty($val['last_update'],'#last_update')){
			echo $o->result();	
			return;
		}

		//mencegah data kosong
		if(!$o->not_empty($val['userid'],'#userid')){
			echo $o->result();	
			return;
		}
		...
		**/
		...
		if($row){
			$data = array(
				'title_page_big'		=> 'Buat Baru',
				'title'					=> $this->title,
				'link_save'				=> site_url().'Digital_channel/Digital_channel/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id_generate,
			);
