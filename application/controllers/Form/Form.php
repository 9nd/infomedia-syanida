<?php
require APPPATH. '/controllers/Form/Form_config.php';
...
class Trans_profiling extends CI_Controller {
   ...
   function __construct(){
		...
		$this->load->model('Trans_profiling_validasi_mos/Trans_profiling_validasi_mos_model','moss_tmodel');
		$this->log_key ='log_Form';
		$this->title = new Form_config();
   }
	...
	public function index(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_refresh_table'	=> site_url().'Form/Form/refresh_table/'.$this->_token,
			'link_create'			=> site_url().'Form/Form/create',
			'link_update'			=> site_url().'Form/Form/update',
			'link_delete'			=> site_url().'Form/Form/delete_multiple',
		);
		
		$this->template->load('Form/Form_list',$data);
	}

	public function refresh_table($token){
		if($token==$this->_token){
			$row = $this->moss_tmodel->json();
			...
			foreach($row['data'] as $val){
				$idgenerate = _encode_id($val['idx'],$tm);
				$row['data'][$x]['idx'] = $idgenerate;
				$x++;
			}
	...
	public function create(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url().'Form/Form/create_action',
			'link_back'				=> $this->agent->referrer(),			
		);
		
		$this->template->load('Form/Form_form',$data);
		...
		**/
		...
		if($row){
			$data = array(
				'title_page_big'		=> 'Buat Baru',
				'title'					=> $this->title,
				'link_save'				=> site_url().'Form/Form/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id_generate,
			);
