...
class Indibox extends CI_Controller
{
	...
	function get_data_list()
	{
		...
		if (isset($_GET['start']) && isset($_GET['end'])) {
			...
			$data['indibox'] = $this->tmodel->live_query("SELECT * FROM trans_profiling_validasi_mos WHERE sumber='IndiBox' AND DATE(tgl_insert) BETWEEN '$start' AND '$end' ")->result_array();
			$data['Status_call_model'] = $this->Status_call_model;
		}
		$this->load->view('Indibox/Indibox_area', $data);
	}
		...
		*/
		...
		if ($success) {
			$data = array(
				...
				'veri_status'	=>  $val['status'],
				'profiling_by'	=>  '100',
				'veri_keterangan'	=>  $val['keterangan'],
