...
class Dashboard_v2 extends CI_Controller
{
  ...
  public function wallboard_reguler_v3()
  {
    ...
    $query_trans_profiling = $this->trans_profiling_daily->live_query(
      "SELECT veri_call,veri_upd,handphone,email,HOUR(lup) as hour_lup FROM trans_profiling_daily WHERE DATE(lup) = '" . $now . "' AND veri_call=13 "
    );
    $total = array();
    for ($i = 8; $i <= 20; $i++) {
      $total['verified'][$i] = 0;
    ...
    $query_trans_profiling = $this->trans_profiling_daily->live_query(
      "SELECT veri_call,veri_upd,handphone,email,HOUR(lup) as hour_lup FROM trans_profiling_daily WHERE DATE(lup) = '" . $now . "' AND (veri_call=1 OR veri_call= 13 OR veri_call=  3 OR veri_call=  12) "
    );
    $total = array();
    for ($i = 8; $i <= 20; $i++) {
      $total['contacted'][$i] = 0;
    ...
    $query_trans_profiling = $this->trans_profiling_daily->live_query(
      "SELECT veri_call,veri_upd,handphone,email,HOUR(lup) as hour_lup FROM trans_profiling_daily WHERE DATE(lup) = '" . $now . "' "
    );
    $total = array();
    for ($i = 8; $i <= 20; $i++) {
      $total['all_call'][$i] = 0;
...
");
    ...
    $data['persen_kenaikan_revenue'] = number_format(($data['revenue'][date('Y-m-d')] / $data['revenue'][$kemarin]) * 100);
    $data['persen_kenaikan_arpu'] = number_format(($data['arpu'][date('Y-m-d')] / $data['arpu'][$kemarin]) * 100);
