...
class Report_qc extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['agree'] = $this->qc->live_query("SELECT id FROM qc WHERE (DATE(tanggal) BETWEEN '$start_filter' AND '$end_filter' ) $where_agent_multi")->num_rows();
		$data['valid'] = $this->qc->live_query("SELECT * FROM qc WHERE status_approve = 1 AND (DATE(tanggal) BETWEEN '$start_filter' AND '$end_filter') $where_agent_multi")->num_rows();
		$data['tdkvalid'] = $this->qc->live_query("SELECT * FROM qc WHERE status_approve = 0 AND (DATE(tanggal) BETWEEN '$start_filter' AND '$end_filter')  $where_agent_multi")->num_rows();
	...
	function get_data_list()
	{
		...
		if (isset($_GET['start']) && isset($_GET['end'])) {
			...
			$data['na'] = $this->qc->live_query("SELECT count(reason_qa) as jml, reason_qa FROM qc WHERE status_approve = 0 AND (DATE( tanggal ) BETWEEN '$start_filter' AND '$end_filter')  $where_agent_multi GROUP BY reason_qa");
			$data['aht'] = $this->qc->live_query("SELECT count(aht_qc) as jml, aht_qc FROM qc WHERE (DATE( tanggal ) BETWEEN '$start_filter' AND '$end_filter')	 $where_agent_multi GROUP BY aht_qc");
			$data['data_tapping'] = $this->qc->live_query("SELECT * FROM qc WHERE  DATE( tanggal ) BETWEEN '$start_filter' AND '$end_filter'  $where_agent_multi  ")->result();
		}
	...
	{
		$id = $_POST['del_id'];
		$cekis = $this->db->query("SELECT COUNT(*) as jml FROM qc_deleted WHERE id='$id'")->row()->jml;
		if ($cekis < 1) {
			$this->db->query("INSERT INTO qc_deleted SELECT * FROM qc WHERE id=$id");
		}
		$cek = $this->db->query("SELECT COUNT(*) as jml FROM qc_deleted WHERE id='$id'")->row()->jml;
		if ($cek == 1) {
			$this->db->query("DELETE FROM qc WHERE id='$id'");
		}
	}
