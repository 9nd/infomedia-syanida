...
class Qcs extends CI_Controller
{
	...
	public function create_action()
	{
		...
		*/
		...
		// if($success && $val['status_approve'] == 1){
		// 	$this->load->library('Telegram');

		// 	$agentid = $val['agentid'];
		// 	$pesan = "Temuan QC Tanggal".$val['tanggal']." 
		// 	reason_qa : ".$val['reason_qa'];
		// 	$query = $this->db->query("SELECT * FROM sys_user WHERE agentid='$agentid'")->row();
		// 	$this->Telegram->send_manual($pesan, $query->agentid, $query->opt_level, $query->chat_id_telegram);
		// }
		// echo $o->auto_result($success);
		// var_dump($val);
			
		$this->Telegram->send_manual(1,1,1,1214868605);
	}
