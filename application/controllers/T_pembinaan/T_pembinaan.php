...
class T_pembinaan extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($data['userdata']->opt_level == 9) {
			$tl = $data['userdata']->agentid;
			$filter_agent = array("opt_level" => 8, "tl =" => "$tl");
			$data['tpembinaan'] = $this->db->query("SELECT * FROM t_pembinaan WHERE jenis IS NULL AND input_by='$tl'")->result_array();
		} else {
			$data['tpembinaan'] = $this->db->query("SELECT * FROM t_pembinaan WHERE jenis IS NULL")->result_array();
			$filter_agent = array("opt_level" => 8, "tl !=" => "-");
		}
		$data['list_agent_d'] = $this->Sys_user_table_model->get_results($filter_agent);
		// var_dump($data);
		$this->template->load('T_pembinaan/T_pembinaan_area', $data);
	}
	...
	public function create_action()
	{
		...
		*/
		...
		if ($success) {
			...
			switch ($val['tingkat_pembinaan']) {
				case "1":
					$tingkat_pembinaan = "Coaching";
					break;
				case "2":
					$tingkat_pembinaan = "Konseling";
					break;
				case "3":
					$tingkat_pembinaan = "BATL";
					break;
				case "4":
					$tingkat_pembinaan = "SP1";
					break;
				case "5":
					$tingkat_pembinaan = "SP2";
					break;
				case "6":
					$tingkat_pembinaan = "SP3";
					break;
				default:
					$tingkat_pembinaan = "Undefined";
			}

			$this->load->library('telegram');

			$agentid = $val['agentid'];
			$pesan = "<b>Update Pembinaan</b> 
Tanggal: " . $val['tanggal_pembinaan'] . " 
TK Pembinaan : " . $tingkat_pembinaan . "
agentid : " . $val['agentid'] . "
