...
class Outbound extends CI_Controller
{
	...
	function index()
	{
		...
		if ($userdata->kategori == "MOS") {
			$data['list_outbound'] = $this->tmodel->live_query("SELECT DISTINCT ncli,no_pstn,no_speedy,nama_pastel,no_handpone,email,layanan,idx FROM
			...
			foreach ($data['list_outbound'] as $datanya) {
				...
				trans_profiling_validasi_mos WHERE `status` IN ('0','3',null) AND update_by is null AND ncli='$ncli' AND no_pstn='$no_pstn' AND layanan<>'TVV'")->result();
