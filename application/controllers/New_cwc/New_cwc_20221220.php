...
class New_cwc extends CI_Controller
{
  ...
  public function report()
  {
    ...
    $reason_call = $_GET['reason_call'];
    $data['reason_call'] = $_GET['reason_call'];
    ...
    if (!isset($reason_call)) {
      $where_reason_call_multi = "";
    } else {
      $where_reason_call_multi = " AND sub_call='$reason_call'";
    }
    $this->template->load('New_cwc/report', $data);
  }
  ...
  public function testwacol()
  {
    $type = $_GET['type'];
    $value = '02158352214';
    if ($value != '') {
      $data['wa_collection'] = $this->wa_collection($type, $value);
    }
    // var_dump($data['wa_collection']);
    $datanya = json_decode($data['wa_collection']);
    // var_dump($datanya);
  ...
  public function multi_contact_list()
  {
    ...
    if ($value != '') {
      ...
      $data['wa_collection'] = $this->wa_collection($type, $value);
      $data['wita'] = $this->wita($type, $value);
    }
  ...
  public function wa_collection($type, $value)
  {
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL => 'http://10.60.175.134/api_dev/index.php/Api/check_blast',
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => '',
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 0,
    ...
    "SND" : "' . $value . '"
      ...
      CURLOPT_HTTPHEADER => array(
        ...
        'Cookie: ci_session=jnobmct5e3sn6ralmm2eah66q7l2ub49'
      ),
    ));
  ...
  public function wita($type, $value)
  {
  ...
  public function wita_test($type, $value)
  {
    ...
    curl_setopt_array($curl, array(
      // CURLOPT_URL => 'http://10.194.176.158/cwctam_v2/api/api_wita.php?function=get_inet_contacted_data_tam&fastel='.$value,
      CURLOPT_URL => 'http://10.194.176.158/cwctam_v2/api/api_wita.php?function=get_inet_contacted_data_tam&fastel=111301132314',
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => '',
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 0,
  ...
  public function report_list()
  {
    ...
    if (isset($_GET['start']) && isset($_GET['end'])) {
      ...
      $userdata = $this->Sys_user_table_model->get_row(array("id" => $logindata->id_user));
      // $data['debug_agent'] = $agentid;
      // $data['debug_start'] = $start_filter;
      if (isset($agentid)) {
        if ($agentid) {
          if (count($_GET['agentid']) > 1) {
            $n_agent_pick = count($_GET['agentid']);
            foreach ($_GET['agentid'] as $k_agentid => $v_agentid) {
              if ($k_agentid == 0) {
                $filter_agent = " AND (veri_upd = '$v_agentid'";
                $where_agent_multi = "AND ( veri_upd = '$v_agentid'";
              } else {
                if ($k_agentid == ($n_agent_pick - 1)) {
                  $where_agent_multi = $where_agent_multi . " OR veri_upd = '$v_agentid' )";
                  $filter_agent = $filter_agent . " OR veri_upd = '$v_agentid' )";
                } else {
                  $where_agent_multi = $where_agent_multi . " OR veri_upd = '$v_agentid' ";
                  $filter_agent = $filter_agent . " OR veri_upd = '$agentid' ";
                }
              }
            }
            $where_agent['or_where_null'] = array($where_agent_multi);
            ...
            if ($agentid[0] != '0') {
              $where_agent['agentid'] = $agentid[0];
              $filter_agent = " AND veri_upd = '$agentid[0]'";
              $where_agent_multi = "AND ( veri_upd = '$agentid[0]')";
            }
          }
        }
      }
      ...
      $data['datanya'] = $this->infomedia->query("SELECT * FROM v2_trans_profiling WHERE  DATE( lup ) BETWEEN '$start_filter' AND '$end_filter'  $where_agent_multi $where_reason_call_multi")->result();
    }
