<?php
require APPPATH. '/controllers/Sys_user_moss/Sys_user_moss_config.php';

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Sys_user_moss extends CI_Controller {
   private $log_key,$log_temp,$title;
   function __construct(){
        parent::__construct();
		$this->load->model('Sys_user_moss/Sys_user_moss_model','tmodel');
		$this->log_key ='log_Sys_user_moss';
		$this->title = new Sys_user_moss_config();
   }
	...
	public function index(){
		$data = array(
			...
			'link_update'			=> site_url().'Sys_user_moss/Sys_user_moss/update',
			'link_delete'			=> site_url().'Sys_user_moss/Sys_user_moss/delete_multiple',
		);
		
		$this->template->load('Sys_user_moss/Sys_user_moss_list',$data);
	}
	...
	public function create(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url().'Sys_user_moss/Sys_user_moss/create_action',
			'link_back'				=> $this->agent->referrer(),			
		);
		
		$this->template->load('Sys_user_moss/Sys_user_moss_form',$data);
	...
	public function create_action(){
		...
		*/	
		...
		//mencegah data kosong
		if(!$o->not_empty($val['periode_start'],'#periode_start')){
			echo $o->result();	
			return;
		}

		//mencegah data kosong
		if(!$o->not_empty($val['periode_end'],'#periode_end')){
			echo $o->result();	
			return;
		}

		//mencegah data kosong
		if(!$o->not_empty($val['shift'],'#shift')){
			echo $o->result();	
			return;
		}
		...
		**/
		...
		if($row){
			$data = array(
				'title_page_big'		=> 'Buat Baru',
				'title'					=> $this->title,
				'link_save'				=> site_url().'Sys_user_moss/Sys_user_moss/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id_generate,
			);
