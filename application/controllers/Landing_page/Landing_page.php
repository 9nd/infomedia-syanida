<?php
require APPPATH . '/controllers/Landing_page/Landing_page_config.php';
...
class Landing_page extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->log_key = 'log_landing_page';
		$this->title = new Landing_page_config();
	}
	...
	{
		$data = array(
			'title_page_big'		=> 'List Tabel Data Landing Page',
			'title'					=> $this->title,
			'link_refresh_table'	=> site_url() . 'Landing_page/Landing_page/refresh_table/' . $this->_token,
			'link_create'			=> site_url() . 'Landing_page/Landing_page/create',
			'link_update'			=> site_url() . 'Landing_page/Landing_page/update',
			'link_delete'			=> site_url() . 'Landing_page/Landing_page/delete_multiple',
		);
		$start = date('Y-m-d');
		$end = date('Y-m-d');
		...
		$this->template->load('Landing_page/Landing_page_list', $data);
	}
	...
	function get_data_list()
	{
		...
		if (isset($_GET['start']) && isset($_GET['end'])) {
			...
			$data['indibox'] = $this->tmodel->live_query("SELECT * FROM trans_profiling_validasi_mos WHERE sumber='LP-Upgrade Spee' AND DATE(tgl_insert) BETWEEN '$start' AND '$end' ")->result_array();
			$data['Status_call_model'] = $this->Status_call_model;
		}
		$this->load->view('Landing_page/Landing_page_area', $data);
	}
	...
	public function detail($id)
	{
		...
		if ($row) {
			$data = array(
				...
				'title'					=> $this->title,
				'link_save'				=> site_url() . 'Landing_page/Landing_page/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id,
			);

			$this->template->load('Landing_page/Landing_page_detail', $data);
		} else {
	...
	public function create($id = false)
	{
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url() . 'Landing_page/Landing_page/create_action',
			'link_back'				=> $this->agent->referrer(),
		);
		$idlogin = $this->session->userdata('idlogin');
		$logindata = $this->log_login->get_by_id($idlogin);
		...
		$this->template->load('Landing_page/Landing_page_form', $data);
	}
		...
		*/
		...
		if ($success) {
			$data = array(
				...
				'veri_status'	=>  $val['status'],
				'profiling_by'	=>  '100',
				'veri_keterangan'	=>  $val['keterangan'] . "/LP-Upgrade Speed",
	...
	public function delete_multiple($idx)
	{
		...
		if ($success) {
			$o->success 	= 'true';
			$o->message	= 'Data berhasil di hapus !';
			redirect(site_url() . "Landing_page/Landing_page/");
		} else {
			$o->success 	= 'false';
			$o->message	= 'Opps..Gagal menghapus data !!';
