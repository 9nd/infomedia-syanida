<?php
require APPPATH . '/controllers/Tvvtl/Tvvtl_config.php';

if (!defined('BASEPATH'))
	exit('No direct script access allowed');

class Tvvtl extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->log_key = 'log_tvvtl';
		$this->title = new Tvvtl_config();
	}
	...
	{
		$data = array(
			'title_page_big'		=> 'List Tabel Data Tvv TL',
			'title'					=> $this->title,
			'link_refresh_table'	=> site_url() . 'Tvvtl/Tvvtl/refresh_table/' . $this->_token,
			'link_create'			=> site_url() . 'Tvvtl/Tvvtl/create',
			'link_update'			=> site_url() . 'Tvvtl/Tvvtl/update',
			'link_delete'			=> site_url() . 'Tvvtl/Tvvtl/delete_multiple',
		);
		$start = date('Y-m-d');
		$end = date('Y-m-d');
		...
		$this->template->load('Tvvtl/Tvvtl_list', $data);
	}
	...
	function get_data_list()
	{
		...
		if (isset($_GET['start']) && isset($_GET['end'])) {
			...
			$data['indibox'] = $this->tmodel->live_query("SELECT * FROM trans_profiling_validasi_mos WHERE sumber='TVVtl' AND DATE(tgl_insert) BETWEEN '$start' AND '$end' ")->result_array();
			$data['Status_call_model'] = $this->Status_call_model;
		}
		$this->load->view('Tvvtl/Tvvtl_area', $data);
	}
	...
	public function detail($id)
	{
		...
		if ($row) {
			$data = array(
				...
				'title'					=> $this->title,
				'link_save'				=> site_url() . 'Tvvtl/Tvvtl/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id,
			);

			$this->template->load('Tvvtl/Tvvtl_detail', $data);
		} else {
	...
	{
		$test = $this->set_mos_to_telkom('51589599', '131159124293', 'Ahmad Sadikin', 'Pemilik', '081221609591', 'ahmadsadikin8888@gmail.com', '', '', '');
	...
	public function create($id = false)
	{
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url() . 'Tvvtl/Tvvtl/create_action',
			'link_back'				=> $this->agent->referrer(),
		);
		$idlogin = $this->session->userdata('idlogin');
		$logindata = $this->log_login->get_by_id($idlogin);
		...
		$this->template->load('Tvvtl/Tvvtl_form', $data);
	}
	...
	public function create_action()
	{
		...
		*/
		...
		$setMos = $this->set_mos_to_telkom($val['ncli'], $val['no_speedy'], $val['nama_pelanggan'], $val['relasi'], $val['no_handpone'], $val['email'],  $val['kota'], $val['facebook'],  $val['twitter']);
		...
		if ($success) {
			$data = array(
				...
				'profiling_by'	=>  '100',
				'veri_keterangan'	=>  $val['keterangan'] . "/TVVsementara",
	...
	public function delete_multiple($idx)
	{
		...
		if ($success) {
			$o->success 	= 'true';
			$o->message	= 'Data berhasil di hapus !';
			redirect(site_url() . "Tvvtl/Tvvtl/");
		} else {
			$o->success 	= 'false';
			$o->message	= 'Opps..Gagal menghapus data !!';
			redirect(site_url() . "Tvvtl//");
		}
