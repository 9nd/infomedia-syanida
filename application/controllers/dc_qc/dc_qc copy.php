...
class dc_qc extends CI_Controller
{
	...
	function get_field($sumber)
	{
		$data = array();
		if ($sumber == "obc") {
			$data['f_hp1'] = 'handphone';
			$data[f_hp2 = 'handphone_lain';
			$data[f_email1 = 'email';
			$data[f_email2 = 'email_lain';
			$data[table = 'trans_profiling_monthly';
			$data[f_date = 'lup';
			$data[qrytmbhn = 'AND veri_call=13';
			...
			$data[f_hp2 = 'NO_HP_1';
			$data[f_email1 = 'EMAIL';
			$data[f_email2 = 'EMAIL_1';
			$data[table = 'dbprofile_verified_temp';
			$data[f_date = 'TGL_VERIFIKASI';
			$data[qrytmbhn = ' ';
		} else {
			$data[f_hp1 = 'handphone';
			$data[f_hp2 = 'handphone_lain';
	...
	function hitung_status($date, $sumber)
	{
		if ($date == "") {
			...
			$data['status']['hp']['contains'] = $this->db->query("SELECT COUNT(*) as jml FROM $table WHERE if($f_hp2 IS NULL, ($f_hp1 LIKE '%+%' OR $f_hp1 LIKE '% %'), ($f_hp1 LIKE '%+%' OR $f_hp1 LIKE '% %' OR $f_hp2 LIKE '%+%' OR $f_hp2 LIKE '% %')) AND DATE($f_date)='$date' $qrytmbhn")->row()->jml;
			$data['status']['hp']['others'] = 0;
	...
	function kosong_e()
	{
		...
		if ($date == "") {
			...
			$data['tabledata'] = $this->db->query("SELECT * FROM $table WHERE ($f_email1 LIKE '%kosong%' OR $f_email2 LIKE '%kosong%') AND DATE($f_date)='$date' $qrytmbhn")->result();
		}
	...
	function invalid_e()
	{
		...
		if ($date == "") {
			...
			$data['tabledata'] = $this->db->query("SELECT * FROM $table WHERE if($f_email2 IS NULL OR $f_email2='', $f_email1 NOT LIKE '%_@_%._%', $f_email1 NOT LIKE '%_@_%._%' OR $f_email2 NOT LIKE '%_@_%._%') AND DATE($f_date)='$date' $qrytmbhn")->result();
		}
	...
	function tdkada_e()
	{
		...
		if ($date == "") {
			...
			$data['tabledata'] = $this->db->query("SELECT * FROM $table WHERE ($f_email1 LIKE '%tdkada%' OR $f_email2 LIKE '%tdkada%') AND DATE($f_date)='$date' $qrytmbhn")->result();
		}
	...
	public function notlike()
	{
		...
		if ($date == "") {
			...
			$data['tabledata'] = $this->db->query("SELECT * FROM $table WHERE if($f_hp2 IS NULL or $f_hp2 = '', (SUBSTR($f_hp1,1,2) NOT LIKE '08%'), (SUBSTR($f_hp1,1,2) NOT LIKE '08%' OR 	SUBSTR($f_hp1,1,2) NOT LIKE '08%')) AND DATE($f_date)='$date' $qrytmbhn")->result();
		}
	...
	public function notnumber()
	{
		...
		if ($date == "") {
			...
			$data['tabledata'] = $this->db->query("SELECT * FROM $table WHERE if($f_hp2 IS NULL, $f_hp1 NOT REGEXP '^[0-9]+$', ($f_hp1 NOT REGEXP '^[0-9]+$' OR $f_hp2 NOT REGEXP '^[0-9]+$')) AND DATE($f_date)='$date' $qrytmbhn")->result();
		}
