...
ini_set('memory_limit', '8192M');
  ...
  public function report_status($filter_condition, $date, $template = "weekly",$veri_call=13)
  {
    ...
    $data['data_performance'] = $this->trans_profiling_daily->live_query(
      "SELECT
        $group_by as bulan,$tabel.veri_call,count(*) as numna 
        FROM
        $tabel
          LEFT JOIN sys_user ON sys_user.agentid = $tabel.veri_upd 
        WHERE
        $where
         AND sys_user.kategori = 'REG' 
          AND sys_user.opt_level = 8 
          $filter_condition
          AND $tabel.veri_call=$veri_call
        GROUP BY
        $group_by,$tabel.veri_call
...
")->result();
  ...
  public function report_weekly($filter_condition, $date, $template = 'weekly')
  {
    ...
    $data['data_performance'] = $this->trans_profiling_daily->live_query(
      "SELECT
        DATE(trans_profiling_last_month.lup) as bulan,
          count(trans_profiling_last_month.idx ) AS oc,
          sum(
          IF
          (trans_profiling_last_month.veri_call IN ( 1, 13, 3, 12, 11 ), 1, 0 )) AS contacted,
          sum(
          IF
          (trans_profiling_last_month.veri_call IN ( 15, 9, 8, 4, 7, 10, 14, 2 ), 1, 0 )) AS not_contacted,
          sum(
        IF
        (trans_profiling_last_month.veri_call = 13, 1, 0 )) AS numna 
        FROM
        trans_profiling_last_month 
          LEFT JOIN sys_user ON sys_user.agentid = trans_profiling_last_month.veri_upd 
        WHERE
        DATE(trans_profiling_last_month.lup) >= '$last_week'
        AND DATE(trans_profiling_last_month.lup) <= '$date'
         AND sys_user.kategori = 'REG' 
          AND sys_user.opt_level = 8 
          $filter_condition
        GROUP BY
        DATE(trans_profiling_last_month.lup)
        ORDER BY trans_profiling_last_month.lup ASC
      "
    )->result();
    $data['summary_performance'] = array();
