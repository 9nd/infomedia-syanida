    ...
    <div class="form-group">
        <div class="selectgroup selectgroup-pills">
            <label class="selectgroup-item">
                ...
                <span class="selectgroup-button selectgroup-button-icon" title="Agent Reguler"><i class="fe fe-user"></i> REGULER</span>
            </label>
            <label class="selectgroup-item">
                <input type="radio" name="icon-input" value="2" class="selectgroup-input">
                <span class="selectgroup-button selectgroup-button-icon" title="Agent MOSS"><i class="fe fe-user"></i> MOSS</span>
            </label>
            <label class="selectgroup-item">
                <input type="radio" name="icon-input" value="3" class="selectgroup-input">
                <span class="selectgroup-button selectgroup-button-icon" title="All Agent"><i class="fe fe-users"></i> ALL</span>
            </label>
        </div>
    </div>
</div>
                        ...
                        <tbody>
                            ...
                            if ($agent_reg['num'] > 0) {
                                ...
                                foreach ($agent_reg['results'] as $ag) {
                                    ...
                                    $status_8 = count($controller->filter_by_value($data_agent, 'veri_call', '8'));
                                    $status_9 = count($controller->filter_by_value($data_agent, 'veri_call', '9'));
                                    $status_15 = count($controller->filter_by_value($data_agent, 'veri_call', '15'));
                                    ...
                                    if ($userdata->opt_level == 8) {
                                        if ($userdata->agentid == $ag->agentid) {
                                            ...
                                            $sub_total_uncontacted = $status_15 +$status_4 + $status_7 + $status_11 + $status_10 + $status_14 + $status_2+$status_8 + $status_9;
                            ...
                            ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo number_format($status_14+$status_8+$status_9); ?></td>
...
<div class="col-md-12 col-xl-12" id="panel-form-all">
    <div class="card">
        <div class="card-status bg-orange"></div>
        <div class="card-header">
            <h3 class="card-title">Report Call All Agent Periode <?php echo date('Y-m-d'); ?>
        ...
        <div class="card-body">
            <div class='box-body table-responsive' id='box-table'>
                <small>
                    <table class='timecard' id="report_table_all" style="width: 100%;">
                        <thead>
                            <tr>
                                <th><b>No</b></th>
                                <th nowrap><b>Nama Agent</b></th>
                        ...
                        <tbody>
                            ...
                            // $check_veri = count($controller->filter_by_value($data_profiling, 'veri_call', '13'));
                            if ($agent_all['num'] > 0) {
                                foreach ($agent_all['results'] as $ag) {
                                    ...
                                    if ($userdata->opt_level == 8) {
                                        if ($userdata->agentid == $ag->agentid) {
                                            $sub_total_contacted = $status_1 + count($verified) + $status_3 + $status_12;
                                            $sub_total_uncontacted = $status_15 + $status_4 + $status_7 + $status_11 + $status_10 + $status_14 + $status_2+$status_9 + $status_8;
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#TLLSN").text('<?php echo number_format($total['TLLSN']); ?>');
        $("#TLAR").text('<?php echo number_format($total['TLAR']); ?>');
        $("#TLNW").text('<?php echo number_format($total['TLNW']); ?>');
        $("#report_table_reg").DataTable();
        $("#report_table_moss").DataTable();
        $("#report_table_all").DataTable();
