                    ...
                    <tbody>
                        ...
                        if (count($data_profiling) > 0) {
                            foreach ($data_profiling as $ag) {
                                $check_approve = $controller->qm_score->get_row(array("idx" => $ag['idx']));
                                $bg_color = "";
                                $status_approve = "";
                                if ($check_approve) {
                                   
                                    $bg_color = "background-color:green !important;color:#ffffff;";
                                    $status_approve = "Sudah Ditapping";
                                    
                                    $link_ = "#";
                                } else {
                                    $status_approve = "Belum Dicheck";
                                    $link_ = "Qm_score/Qm_score/form_approve?agentid=" . $ag['veri_upd'] . "&ncli=" . $ag['ncli'] . "&handphone=" . $ag['handphone'] . "&start=" . $_GET['start'] ;
                                }
                        ?>
