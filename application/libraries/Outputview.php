...
class Outputview
{
    ...
    {
        // $this->CI->load->model('Custom_model/Sys_message_model', 'sys_message');
        // $this->CI->load->model('sys/Sys_user_log_model', 'log_login');
        // $idlogin = $this->CI->session->userdata('idlogin');
        // $logindata = $this->CI->log_login->get_by_id($idlogin);
        // $num_message=$this->CI->sys_message->live_query("SELECT id FROM sys_message WHERE status_read=0 AND to_user_id = '$logindata->id_user' GROUP BY group_id")->num_rows();
        $num_message = 0;
        return $num_message;
    }
}
